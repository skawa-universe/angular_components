...
DatepickerDateRange? _ifValidRange(DatepickerDateRange? r, DatepickerDateRange fn()) {
...
DatepickerDateRange? _genericNext(DatepickerDateRange? r) => _ifValidRange(
    r, () => DatepickerDateRange.custom(r!.end!.add(days: 1), r.end!.add(days: daysSpanned(r.start!, r.end!))));

DatepickerDateRange? _genericPrev(DatepickerDateRange? r) => _ifValidRange(
    r, () => DatepickerDateRange.custom(r!.start!.add(days: -daysSpanned(r.start!, r.end!)), r.start!.add(days: -1)));
...
abstract class DatepickerDateRange implements DateRange {
  ...
  factory DatepickerDateRange.custom(Date? start, Date? end) => DatepickerDateRange(_customDateRangeMsg, start, end);

  /// Construct from the protocol buffer format defined in date_range.proto.
  factory DatepickerDateRange.fromProtoBuf(Clock clock, proto.DatepickerDateRange pb) {
    if (pb.hasAllTimeRange()) {
      ...
      if (pb.hasStartWeekday()) {
        return WeekRange.weeksAgo(clock, pb.weeksAgo, startWeekday: pb.startWeekday);
      } else {
    ...
    return DatepickerDateRange.custom(_convertToDate(pb.dateRange.start), _convertToDate(pb.dateRange.end));
  }
}
...
int rangeHash(DatepickerDateRange range) => range.title.hashCode ^ range.start.hashCode ^ range.end.hashCode;

String _rangeString(DatepickerDateRange range) => '${range.title} (${range.start}) (${range.end})';

DateRange _plainRange(DatepickerDateRange range) => DateRange(range.start, range.end);

proto.DatepickerDateRange _makeProtoBuf(DatepickerDateRange range) => range.isAllTime
    ? (proto.DatepickerDateRange()..allTimeRange = true)
    : (proto.DatepickerDateRange()..dateRange = _makeDateRangeProto(range.start!, range.end!));
...
Date _convertToDate(dateproto.Date date) => Date(date.year, date.month, date.day);
...
class _ClampedDateRange implements DatepickerDateRange {
  ...
  @override
  DatepickerDateRange? clamp({Date? min, Date? max}) => _clamp(this, min: min, max: max);
  ...
  @override
  proto.DatepickerDateRange toProtoBuf() => _delegate.toProtoBuf()..dateRange = _makeDateRangeProto(start!, end!);

  bool operator ==(o) => rangeEqual(this, o) && (o is _ClampedDateRange) && _min == o._min && _max == o._max;
...
class _BasicDateRange implements DatepickerDateRange {
  ...
  _BasicDateRange(this.title, this.start, this.end, this._next, this._prev, this.isPredefined, this.isAllTime);
...
class SingleDayRange implements DatepickerDateRange {
  ...
  DatepickerDateRange get next => SingleDayRange(_date.add(days: 1), _ago - 1, _titleFunction);

  DatepickerDateRange get prev => SingleDayRange(_date.add(days: -1), _ago + 1, _titleFunction);
  ...
  static String _defaultTitle(int daysAgo) => daysAgo > 0 ? _addDaysMsg(daysAgo) : _daysFromNowMsg(-daysAgo);
...
class LastNDaysRange extends MultipleDaysRange {
  ...
  LastNDaysRange.beforeToday(clock, lengthInDays, [String? title])
      : super(Date.today(clock).add(days: -lengthInDays), lengthInDays, title ?? _lastNDaysMsg(lengthInDays));

  @override
  proto.DatepickerDateRange toProtoBuf() => _makeProtoBuf(this)..lastNDays = _lengthInDays;
...
class LastNDaysToTodayRange extends MultipleDaysRange {
  ...
  proto.DatepickerDateRange toProtoBuf() => _makeProtoBuf(this)..lastNDaysToToday = _lengthInDays;

  static String _lastNDaysToTodayMsg(int lengthInDays) => Intl.plural(lengthInDays,
      one: 'Today',
...
class NextNDaysFromTodayRange extends MultipleDaysRange {
  ...
  NextNDaysFromTodayRange.afterToday(clock, lengthInDays, [String? title])
      : super(Date.today(clock), lengthInDays, title ?? _nextNDaysFromTodayMsg(lengthInDays));

  proto.DatepickerDateRange toProtoBuf() => _makeProtoBuf(this)..nextNDaysFromToday = _lengthInDays;

  static String _nextNDaysFromTodayMsg(int lengthInDays) => Intl.plural(lengthInDays,
      one: 'Today',
...
class WeekRange implements DatepickerDateRange {
  ...
  WeekRange(this._start, this._ago, [this._titleFunction = _defaultTitle, this._startWeekday]);
  ...
  /// current locale.
  WeekRange.weeksAgo(clock, ago, {RangeTitle titleFunction = _defaultTitle, int? startWeekday})
      : this(
            _weekStart(Date.today(clock), startWeekday ?? (DateFormat().dateSymbols.FIRSTDAYOFWEEK + 1))
  ...
  DatepickerDateRange get next => WeekRange(_start.add(days: 7), _ago - 1, _titleFunction);

  DatepickerDateRange get prev => WeekRange(_start.add(days: -7), _ago + 1, _titleFunction);
  ...
  static String _defaultTitle(int weeksAgo) => weeksAgo > 0 ? _weeksAgoMsg(weeksAgo) : _weeksFromNowMsg(-weeksAgo);

  static String _weeksAgoMsg(int weeksAgo) => Intl.plural(weeksAgo,
      zero: 'This week',
      // TODO(google): 'This week (Mon-Sun)'
      one: 'Last week',
...
class MonthRange implements DatepickerDateRange {
  ...
  MonthRange(Date start, this._ago, [this._titleFunction = _defaultTitle]) : _start = Date(start.year, start.month, 1);

  MonthRange.monthsAgo(Clock clock, int ago, [RangeTitle titleFunction = _defaultTitle])
  ...
  DatepickerDateRange get next => MonthRange(_start.add(months: 1), _ago - 1, _titleFunction);

  DatepickerDateRange get prev => MonthRange(_start.add(months: -1), _ago + 1, _titleFunction);
  ...
  proto.DatepickerDateRange toProtoBuf() => _makeProtoBuf(this)..monthsAgo = _ago;
  ...
  static String _monthsFromNowMsg(int monthsFromNow) => Intl.plural(monthsFromNow,
      zero: 'This month',
      one: 'Next month',
...
class BroadcastMonthRange implements DatepickerDateRange {
  ...
  BroadcastMonthRange._(this._theFirst, this._ago, [this._titleFunction = _defaultTitle]);

  factory BroadcastMonthRange.monthsAgo(Clock clock, int ago, [RangeTitle titleFunction = _defaultTitle]) {
    ...
    var theFirst =
        nextCalendarMonthTheFirst.isAfter(_weekEnd(today)) ? thisCalendarMonthTheFirst : nextCalendarMonthTheFirst;
    return BroadcastMonthRange._(theFirst.add(months: -ago), ago, titleFunction);
  }
  ...
  DatepickerDateRange get next => BroadcastMonthRange._(_theFirst.add(months: 1), _ago - 1, _titleFunction);

  DatepickerDateRange get prev => BroadcastMonthRange._(_theFirst.add(months: -1), _ago + 1, _titleFunction);
  ...
  proto.DatepickerDateRange toProtoBuf() => _makeProtoBuf(this)..broadcastMonthsAgo = _ago;
  ...
  static String _broadcastMonthsAgoMsg(broadcastMonthsAgo) => Intl.plural(broadcastMonthsAgo,
  ...
  static String _broadcastMonthsFromNowMsg(broadcastMonthsFromNow) => Intl.plural(broadcastMonthsFromNow,
      zero: 'This broadcast month',
...
class YearRange implements DatepickerDateRange {
  ...
  YearRange(Date start, this._ago, [this._titleFunction = _defaultTitle]) : _start = Date(start.year, 1, 1);
  ...
  DatepickerDateRange get next => YearRange(_start.add(years: 1), _ago - 1, _titleFunction);

  DatepickerDateRange get prev => YearRange(_start.add(years: -1), _ago + 1, _titleFunction);
  ...
  proto.DatepickerDateRange toProtoBuf() => _makeProtoBuf(this)..yearsAgo = _ago;
  ...
  static String _defaultTitle(int yearsAgo) => yearsAgo > 0 ? _yearsAgoMsg(yearsAgo) : _yearsFromNowMsg(-yearsAgo);
...
class QuarterRange implements DatepickerDateRange {
  ...
  QuarterRange.quartersAgo(Clock clock, int ago, [RangeTitle titleFunction = _defaultTitle])
  ...
  @override
  DatepickerDateRange get next => QuarterRange(start.add(months: 3), _ago - 1, _titleFunction);

  @override
  DatepickerDateRange get prev => QuarterRange(start.add(months: -3), _ago + 1, _titleFunction);
  ...
  @override
  proto.DatepickerDateRange toProtoBuf() => _makeProtoBuf(this)..quartersAgo = _ago;
  ...
  static String _defaultTitle(int quartersAgo) =>
      quartersAgo > 0 ? _quartersAgoMsg(quartersAgo) : _quartersFromNowMsg(-quartersAgo);
  ...
  static String _quartersFromNowMsg(int quartersFromNow) => Intl.plural(quartersFromNow,
      zero: 'This quarter',
      one: 'Next quarter',
...
DatepickerDateRange last7Days(Clock clock) => LastNDaysRange.beforeToday(clock, 7);

DatepickerDateRange last14Days(Clock clock) => LastNDaysRange.beforeToday(clock, 14);
...
DatepickerDateRange thisBroadcastMonth(Clock clock) => BroadcastMonthRange.monthsAgo(clock, 0);

DatepickerDateRange lastBroadcastMonth(Clock clock) => BroadcastMonthRange.monthsAgo(clock, 1);

DatepickerDateRange last30Days(Clock clock) => LastNDaysRange.beforeToday(clock, 30);
...
DatepickerDateRange thisQuarter(Clock clock) => QuarterRange.quartersAgo(clock, 0);

DatepickerDateRange lastQuarter(Clock clock) => QuarterRange.quartersAgo(clock, 1);
...
DatepickerDateRange allTime = DatepickerDateRange(_allTimeMsg, null, null, isPredefined: true, isAllTime: true);
...
final _allTimeMsg =
    Intl.message('All time', name: '_allTimeMsg', desc: 'A date range containing the entire lifetime of the account.');
