...
class DatepickerComparison implements DateRangeComparison {
  ...
  DatepickerComparison.noComparison(DatepickerDateRange? range) : this.custom(range, null);

  DatepickerComparison.previousPeriod(DatepickerDateRange? range) : this(range, ComparisonOption.previousPeriod);

  DatepickerComparison.samePeriodLastYear(DatepickerDateRange? range) : this(range, ComparisonOption.samePeriodLastYear);
  ...
  bool comparesTo(ComparisonOption? option) =>
      ...
      option != null &&
      range != null &&
      comparison!.unclamped() == option.computeComparisonRange(range!.unclamped());

  bool operator ==(o) =>
      o is DatepickerComparison && rangeEqual(range, o.range) && rangeEqual(comparison, o.comparison);

  int get hashCode => isComparisonEnabled ? rangeHash(range!) ^ rangeHash(comparison!) : rangeHash(range!);
