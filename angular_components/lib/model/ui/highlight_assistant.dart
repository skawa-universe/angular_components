...
class HighlightAssistant {
  ...
  final _highlightCache = <String, Map<dynamic, List<HighlightedTextSegment>>>{};
  ...
  TextHighlighter get _textHighlighter =>
      __textHighlighter ??= TextHighlighter(matchFromStartOfWord: _matchFromStartOfWord);
  ...
  List<HighlightedTextSegment> highlightOption<T>(String _lastQuery, dynamic item, ItemRenderer<T> itemRenderer) {
    ...
    if (value == null) {
      value = (_optionHighlighter != null
          ...
          : _textHighlighter.highlight(itemRenderer(item), _lastQuery.split(_separatorRegex)));
