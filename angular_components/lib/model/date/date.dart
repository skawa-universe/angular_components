...
class Date extends Comparators<Date> {
  ...
  Date(int year, [int month = 1, int day = 1]) : _time = DateTime.utc(year, month, day);
  ...
  factory Date.fromTimestamp(int msSinceEpoch) => Date.fromTime(DateTime.fromMillisecondsSinceEpoch(msSinceEpoch));
  ...
  factory Date.parse(String input, DateFormat format) => Date.fromTime(format.parse(input));

  factory Date.parseStrict(String input, DateFormat format) => Date.fromTime(format.parseStrict(input));

  factory Date.parseLoose(String input, DateFormat format) => Date.fromTime(format.parseLoose(input));
  ...
  Date subtract({int years = 0, int months = 0, int days = 0}) => add(years: -years, months: -months, days: -days);
  ...
  @override
  String toString() => '${asUtcTime().year}-${asUtcTime().month}-${asUtcTime().day}';
}
...
int daysSpanned(Date start, Date end, {bool inclusive = true}) =>
    ...
    (end.asUtcTime().difference(start.asUtcTime()).inHours / 24).round() + (inclusive ? 1 : 0);
...
bool rangeContainsRange(DateRange range, DateRange subrange) {
  ...
  final containsEnd =
      (range.end == null && subrange.end == null) || (subrange.end != null && rangeContains(range, subrange.end!));
...
Iterable<Date> enumerateRange(DateRange? range) {
  ...
  if (range.start == null || range.end == null) {
    throw ArgumentError('Cannot call enumerateRange with a range with a null start or end.');
  }
...
class DatepickerSelection extends ObservableViewMixin<DateRangeComparison> {
  ...
  DatepickerSelection.test([DateRangeComparison? initialValue]) : _ref = ObservableReference(initialValue);
