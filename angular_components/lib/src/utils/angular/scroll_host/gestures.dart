...
class GestureListenerFactory {
  ...
  GestureListener create(Element element, DirectionCheck isDirectionScrollable) =>
      GestureListener(element, isDirectionScrollable, _clock);
}

/// Directions in which a parent of [target] can scroll within the [host].
Map<GestureDirection, bool> innerScrollableDirections(Element host, EventTarget target) {
  ...
  while (element != host && element != null) {
    ...
    if (overflowX == 'auto' || overflowX == 'scroll') {
      directions[GestureDirection.left] = directions[GestureDirection.left]! || element.scrollLeft > 0;
      directions[GestureDirection.right] =
          directions[GestureDirection.right]! || element.scrollLeft + element.clientWidth < element.scrollWidth;
    }
    String overflowY = style.getPropertyValue('overflow-y');
    if (overflowY == 'auto' || overflowY == 'scroll') {
      directions[GestureDirection.up] = directions[GestureDirection.up]! || element.scrollTop > 0;
      directions[GestureDirection.down] =
          directions[GestureDirection.down]! || element.scrollTop + element.clientHeight < element.scrollHeight;
    }
...
class GestureEvent extends ScrollHostEventImpl {
  ...
  GestureEvent(int deltaX, int deltaY, this.startingTarget) : super(deltaX, deltaY);
}
...
class GestureListener implements Disposable {
  ...
  Stream<GestureEvent> get scrollStream {
    if (_scrollController == null) {
      _scrollController = StreamController<GestureEvent>.broadcast(onListen: _startListeners, onCancel: _onCancel);
    }
  ...
  void _startListeners() {
    ...
    _disposer!.addStreamSubscription(_element.onTouchStart.listen(_onTouchStart));
...
class _Gesture {
  ...
  late Point _startPoint;
  late Point _lastTouchPoint;
  late Point _lastSyncPoint;
  bool _finished = false;
  late EventTarget _startingTarget;
  ...
  void _addFlingEvent(Timer _) {
    ...
    int deltaX = (speed * _velocityX.sign * _scrollInterval.inMilliseconds).round();
    int deltaY = (speed * _velocityY.sign * _scrollInterval.inMilliseconds).round();
