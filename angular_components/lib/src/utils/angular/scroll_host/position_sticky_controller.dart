...
class PositionStickyController implements StickyController {
  ...
  void stick(Element element, StickyPosition position, Element? range, {String? stickyClass, String? stickyKey}) {
    final stickyElement = _StickyElement(element, position, range, stickyClass, stickyKey);
    if (_stickyElements.contains(stickyElement)) return;

    // Prevent the element from appearing in the layout more than once.
    _stickyElements.removeWhere((stickyElement) => element == stickyElement.element);
  ...
  @override
  Stream<Null> get onUpdate => throw UnsupportedError('PositionStickyController.onUpdate');
  ...
  void _addStickyStyle(_StickyElement stickyElement, String positionProperty, num zIndex, num offset) {
    ...
    if (stickyElement.stickyClass != null) {
      if (feature_detector.supportsIntersectionObserver) {
        ...
        stickyElement.intersectionElement!.style.setProperty(positionProperty, '${-(offset + 2)}px');
      } else {
  ...
  void _startIntersectionSubscription(_StickyElement stickyElement) {
    ...
    stickyElement.intersectionSubscription = _scrollHost.onIntersection(stickyElement.intersectionElement!).listen((e) {
  ...
  void _update() {
    var topElements = _stickyElements.where((e) => e.position == StickyPosition.TOP).toList();
    var bottomElements = _stickyElements.where((e) => e.position == StickyPosition.BOTTOM).toList();
  ...
  int _updateLayout(List<_StickyElement> elements, int sortOrder, String positionProperty, int startZIndex) {
    // Create a list of sticky elements and corresponding client rect, sorted by
    // the element's position in the document tree.
    var elementsAndRects = elements.map((e) => [e, e.element.getBoundingClientRect()]).toList();
    elementsAndRects.sort((a, b) {
      _StickyElement elementA = a[0] as _StickyElement;
      _StickyElement elementB = b[0] as _StickyElement;
      return sortOrder * compareDocumentPosition(elementA.element, elementB.element);
    });
    ...
    for (var item in elementsAndRects) {
      ...
      if (stickyElement.stickyKey != null) {
        ...
        _addStickyStyle(stickyElement, positionProperty, zIndex, stickyKeyOffsets[stickyElement.stickyKey]!);
      } else {
...
class _StickyElement {
  ...
  _StickyElement(this.element, this.position, this.range, this.stickyClass, this.stickyKey);
  ...
  @override
  int get hashCode => hashObjects([element, position, range, stickyClass, stickyKey]);
}
