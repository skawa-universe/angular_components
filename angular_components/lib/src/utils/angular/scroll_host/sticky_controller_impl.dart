...
class StickyControllerImpl implements StickyController {
  ...
  @override
  void stick(Element element, StickyPosition position, Element? range, {String? stickyClass, String? stickyKey}) {
    _StickyRow? row = _rowMap[element];
    // if the definition's key fields are the same, assume that nothing changed
    if ((row != null) && (row.position == position) && (row.range == range) && (row.stickyKey == stickyKey)) {
      return;
    }
    unstick(element);
    _rowMap[element] = _StickyRow(element, position, range, stickyClass, stickyKey);
    ...
    for (Element element in _floatingElements) {
      ...
      if (topDistance < bottomDistance) {
        ...
        if ((newTop != hostRect.top) && (newHeight > 0)) {
          hostRect = Rectangle(hostRect.left, newTop, hostRect.width, newHeight);
        }
      } else {
        num newBottom = min(hostRect.bottom, rect.top);
        num newHeight = newBottom - hostRect.top;
        if ((newBottom != hostRect.bottom) && (newHeight > 0)) {
          hostRect = Rectangle(hostRect.left, hostRect.top, hostRect.width, newHeight);
        }
      }
    }
    return Rectangle(hostRect.left, hostRect.top, hostRect.width, hostRect.height);
  }
  ...
  StickyContainerLayout<_StickyRow> _getLayout() {
    ...
    return StickyRowUtils.calculateLayout<_StickyRow>(hostPosition, _orderedRows!,
  ...
  void _observeRowPositions() {
    ...
    _orderedRows!.sort((a, b) {
      ...
      return a.isTop ? a.rowPosition.top.compareTo(b.rowPosition.top) : b.rowPosition.top.compareTo(a.rowPosition.top);
    });
  }
...
class _StickyRow implements StickyRowPosition {
  ...
  _StickyRow(this.element, this.position, this.range, this.stickyClass, this._stickyKey) {
  ...
  void readRowPositions() {
    ...
    if (translateY != null) {
      rowPosition = Rectangle(rowPosition.left, rowPosition.top - translateY!, rowPosition.width, rowPosition.height);
    }
  ...
  void moveToTop(num top) {
    ...
    if (translateY != newY) {
      ...
      if (_currentPosition != 'relative' || _currentTransform != newTransform || _currentZIndex != newZIndex) {
        element.style
          ..position = 'relative'
...
class _RowData<T> {
  ...
  bool operator ==(Object other) =>
      ...
      other is _RowData && runtimeType == other.runtimeType && row == other.row && offsetY == other.offsetY;
...
abstract class StickyRowUtils {
  ...
  static bool shouldStick(bool isTop, num hostTop, num hostBottom, Rectangle rowPosition, Rectangle? rangePosition) {
    if (rowPosition.height == 0) {
      return false;
    }
    if (isTop) {
      // the range, if it exists, is still visible or below the bottom
      bool rangeVisible = rangePosition == null || rangePosition.top > (hostTop + rowPosition.height);
      return
      ...
      bool rangeVisible = rangePosition == null || rangePosition.bottom < (hostBottom - rowPosition.height);
      return
  ...
  static StickyContainerLayout<T> calculateLayout<T extends StickyRowPosition>(Rectangle hostPosition, List<T> rows,
      {bool enableSmoothPushing = false}) {
    ...
    for (var i = 0; i < rows.length; i++) {
      var row = rows[i];
      bool shouldStick = StickyRowUtils.shouldStick(row.isTop, hostTop, hostBottom, row.rowPosition, row.rangePosition);
      ...
      bool hasStuckDuplicate =
          shouldStick && row.stickyKey != null && stickyKeyToRowIndex?.containsKey(row.stickyKey) == true;
      ...
      if (hasStuckDuplicate && row.isTop) {
        ...
        shouldStick = StickyRowUtils.shouldStick(
            row.isTop, hostTop - duplicateRow.row.rowPosition.height, hostBottom, row.rowPosition, row.rangePosition);
      }
      ...
      if (shouldStick && !hasStuckDuplicate) {
        ...
        if (shouldReplace) {
          final duplicateRow = layout.topRows![stickyKeyToRowIndex![row.stickyKey]!].row;
          layout.hiddenRows!.add(duplicateRow);
          layout.topRows![stickyKeyToRowIndex[row.stickyKey]!] = _RowData(row, offsetY: 0);
      ...
      if (enableSmoothPushing && !shouldStick && hasStuckDuplicate && row.isTop) {
