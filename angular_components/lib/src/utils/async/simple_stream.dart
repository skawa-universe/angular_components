...
typedef SubscriptionChangeListener<T> = void Function(StreamSubscription<T> subscription);
...
class SimpleStream<T> extends Stream<T> implements EventSink<T> {
  ...
  void _sendItem(List<SimpleStreamSubscription<T>> listeners, T item, [int len = -1]) {
    ...
    for (int i = 0; i < len; i++) {
      ...
      if (callback != null) {
        if (callback is ZoneBinaryCallback<Object, Object, Object>) {
          callback(errorEvent, stackTrace as Object);
  ...
  StreamSubscription<T> listen(void onData(T event)?, {Function? onError, void onDone()?, bool? cancelOnError}) {
    ...
    var sub = SimpleStreamSubscription<T>(this, onData, onDone, onError, cancelOnError ?? false, contextZone);
...
class LastStateStream<T> extends SimpleStream<T> {
  ...
  LastStateStream(
      ...
      : super.broadcast(isSync: isSync, runInZone: runInZone, onListen: onListen, onCancel: onCancel);
  ...
  @override
  StreamSubscription<T> listen(void onData(T event)?, {Function? onError, void onDone()?, bool? cancelOnError}) {
    SimpleStreamSubscription<T> sub = super
        .listen(onData, onError: onError, onDone: onDone, cancelOnError: cancelOnError) as SimpleStreamSubscription<T>;
    if (_lastItem != null) {
...
class SimpleStreamSubscription<T> implements StreamSubscription<T> {
  ...
  factory SimpleStreamSubscription._empty() => SimpleStreamSubscription(null, null, null, null, false, null);

  SimpleStreamSubscription(
      this._stream, this._callback, this._doneCallback, this._onError, this._cancelOnError, this._contextZone);
