...
class CalendarMonth {
  ...
  CalendarMonth(int year, int month, {CalendarState? state, this.startingWeekday = DateTime.monday})
  ...
  CalendarMonth.fromDate(Date date, {CalendarState? state, int startingWeekday = DateTime.monday})
      : this(date.year, date.month, state: state, startingWeekday: startingWeekday);

  CalendarMonth.fromTime(DateTime time, {CalendarState? state, int startingWeekday = DateTime.monday})
      : this(time.year, time.month, state: state, startingWeekday: startingWeekday);
  ...
  void update(CalendarState? state) {
    ...
    if (state == null || state.resolution != CalendarResolution.months) {
      ...
      _classes = state.selections
          .where((selection) => containsDate(selection.start!) || containsDate(selection.end!))
  ...
  CalendarMonth addMonths(int months) {
    ...
    return CalendarMonth(newYear, newMonth, state: calendarState, startingWeekday: startingWeekday);
  }
  ...
  int deltaMonths(CalendarMonth other) => (other.year - year) * 12 + other.month - month;
...
class MonthRange {
  ...
  factory MonthRange.within(CalendarMonth? min, CalendarMonth? max, int length, {final CalendarMonth? tryToStartAt}) {
    if (length < 1) {
  ...
  List<CalendarMonth> toList(CalendarState state, int startingWeekday) {
    var startMonth = CalendarMonth(start.year, start.month, state: state, startingWeekday: startingWeekday);
