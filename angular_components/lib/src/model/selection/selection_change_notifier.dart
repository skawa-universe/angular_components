...
abstract class SelectionChangeNotifier<T> implements SelectionModel<T> {
  ...
  @override
  bool deliverSelectionChanges() {
    if (hasSelectionObservers && _selectionChangeRecords != null && _selectionChangeRecords!.isNotEmpty) {
      var records = UnmodifiableListView<SelectionChangeRecord<T>>(_selectionChangeRecords!);
  ...
  @override
  void notifySelectionChange({Iterable<T> added = const [], Iterable<T> removed = const []}) {
  ...
  @override
  bool get hasSelectionObservers {
    return _selectionChangeController != null && _selectionChangeController!.hasListener;
  }

  @override
  Stream<List<SelectionChangeRecord<T>>> get selectionChanges {
    if (_selectionChangeController == null) {
      _selectionChangeController = StreamController<List<SelectionChangeRecord<T>>>.broadcast(sync: true);
    }
...
class _SelectionChangeRecordImpl<T> extends ChangeRecord implements SelectionChangeRecord<T> {
  @override
  ...
  factory _SelectionChangeRecordImpl({Iterable<T>? added, Iterable<T>? removed}) {
    added = added != null ? UnmodifiableListView(added) : <T>[];
    removed = removed != null ? UnmodifiableListView(removed) : <T>[];
  ...
  @override
  String toString() => 'SelectionChangeRecord{added: $added, removed: $removed}';
}
